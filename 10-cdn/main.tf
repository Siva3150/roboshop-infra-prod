resource "aws_cloudfront_distribution" "sivadevops" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.sivadevops.domain_name
        zone_id = aws_cloudfront_distribution.sivadevops.hosted_zone_id
      }
    }
  ]
}